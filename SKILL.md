...
S2 的 matching 题（Q15-20 等）通过 `生成 fill → 后处理转换` 的方式实现。

```bash
# 1. 先生成标准 fill 类型 HTML
python scripts/generate_player.py ...

# 2. 再用后处理脚本转换为拖拽匹配
python scripts/postprocess/apply_matching.py output.html
```

### Matching 布局规则（MANDATORY）

1. **主容器 max-width:** 1280px，`margin:0 auto` 居中
2. **左右分栏：** 左侧题目区（~78%）+ 右侧选项池（~22%，min-width 280px）
3. **选项文本保护：** `white-space:nowrap` 防止断行
4. **Drop slot 宽度：** 180px，`flex-shrink:0`
5. **交互方式：** Click-to-place 为主（`optClick` 选中 → `slotClick` 放入）+ Swap（点击已填 slot 交换选项）
6. **响应式：** ≤750px 时堆叠为上下布局
